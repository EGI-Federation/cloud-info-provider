...
other supported *formatters* include GLUE 2.1 Schema and CMDB. Use the
positional argument of cloud-info-provider-service command (allowed values:
...
GLUE 2.1 Schema output can be generated by using the positional argument
`glue21`, e.g. using OpenStack provider:

```sh
cloud-info-provider-service glue21 \
    --middleware openstack \
    --os-username <username> --os-password <password> \
    --os-user-domain-name default \
    --os-project-name <tenant> \
...
is generated by using the positional argument `cmdb`, e.g. using OpenStack
provider:

```sh
cloud-info-provider-service cmdb \
    --middleware openstack \
    --os-username <username> --os-password <password> \
    --os-user-domain-name default \
    --os-project-name <tenant> \
