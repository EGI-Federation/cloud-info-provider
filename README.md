...
Current supported cloud middleware:
...
based distros it is in EPEL, and in the backports repo for Debian wheezy).
...
`--yaml-file` option. The file has two main maps:
...
For generating GlueSchema2.1 use `glue` as extension.
...
GlueSchema2.1 output can be generated by using the `--template-extension glue21`
