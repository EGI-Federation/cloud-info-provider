...
class OpenNebulaBaseProvider(base.BaseProvider):
    def __init__(self, opts, **kwargs):
        ...
        if not self.on_auth:
            msg = (
                "ERROR, You must provide a on_auth "
        ...
        if not self.on_rpcxml_endpoint:
            msg = (
                "You must provide an OpenNebula RPC-XML "
    ...
    def _get_one_templates(self):
        response = self.server_proxy.one.templatepool.info(self.on_auth, -3, -1, -1)
        return self._handle_response(response)

    def _get_one_images(self):
        response = self.server_proxy.one.imagepool.info(self.on_auth, -3, -1, -1)
        return self._handle_response(response)
    ...
    def get_images(self, **kwargs):
        ...
        for tpl_id, tpl in self._get_one_templates().items():
            ...
            aux_tpl.update(
                {
                    "image_marketplace_id": template.get("cloudkeeper_appliance_mpuri"),
                    "image_description": template.get(
                        "cloudkeeper_appliance_description"
                    ),
                    "image_version": template.get("cloudkeeper_appliance_version"),
            ...
            if template.get("cloudkeeper_appliance_base_mpuri"):
                ...
                ck_attrs = json.loads(base64.b64decode(encoded_attrs).decode("utf-8"))
            ...
            if template.get("cloudkeeper_appliance_traffic_in"):
                aux_tpl["network_traffic_in"] = utils.pythonize_network_info(
                    template["cloudkeeper_appliance_traffic_in"]
                )

            if template.get("cloudkeeper_appliance_traffic_out"):
                aux_tpl["network_traffic_out"] = utils.pythonize_network_info(
                    template["cloudkeeper_appliance_traffic_out"]
                )
    ...
    def populate_parser(parser):
        parser.add_argument(
            ...
            help=(
                "Specify authorization information. For core drivers, "
        ...
        parser.add_argument(
            ...
            help=(
                "Specify OpenNebula XML RPC endpoint. "
    ...
    def _recurse_dict(element):
        return (
            element.tag.lower(),
            dict(map(OpenNebulaProvider._recurse_dict, element)) or element.text,
        )
...
class OpenNebulaROCCIProvider(OpenNebulaBaseProvider):
    ...
    def __init__(self, opts, **kwargs):
        ...
        if not self.rocci_template_dir and not self.rocci_remote_templates:
            msg = (
                "ERROR, You must provide a rocci_template_dir "
    ...
    def local_templates(self, template):
        ...
        for mxn_file in self._absolute_file_paths(self.rocci_template_dir):
            ...
            disk = (
                self._traverse(occi_attrs, "ephemeral_storage", "size", "Default") or 0
            )
    ...
    @staticmethod
    def populate_parser(parser):
        super(OpenNebulaROCCIProvider, OpenNebulaROCCIProvider).populate_parser(parser)
        ...
        parser.add_argument(
            ...
            default=False,
            help=(
                "If set, resource template definitions will be retrieved "
