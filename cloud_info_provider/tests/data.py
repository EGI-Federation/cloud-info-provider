...
class Data(object):
    @property
    def site_info(self):
        return {"suffix": "o=glue", "site_name": "SITE_NAME"}
...
class OpenNebulaFakes(object):
    def __init__(self):
        ...
        with open(os.path.join(sdir, "opennebula_base_provider_images.json"), "r") as f:
            self.opennebula_base_provider_expected_images = yaml.safe_load(f)

        with open(
            os.path.join(sdir, "opennebula_rocci_provider_images.json"), "r"
        ) as f:
            self.opennebula_rocci_provider_expected_images = yaml.safe_load(f)

        with open(
            os.path.join(sdir, "opennebula_rocci_provider_templates.json"), "r"
        ) as f:
            self.opennebula_rocci_provider_expected_templates = yaml.safe_load(f)

        jsonf = os.path.join(sdir, "opennebula_rocci_provider_templates_remote.json")
        with open(jsonf, "r") as f:
            self.opennebula_rocci_provider_expected_templates_remote = yaml.safe_load(f)

        with open(os.path.join(sdir, "indigo_on_provider_images.json"), "r") as f:
            self.indigo_on_provider_expected_images = yaml.safe_load(f)

        with open(os.path.join(sdir, "indigo_on_provider_templates.json"), "r") as f:
