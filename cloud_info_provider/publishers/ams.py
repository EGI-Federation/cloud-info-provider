...
class AMSPublisher(BasePublisher):
    ...
    def populate_parser(parser):
        parser.add_argument(
            "--ams-token",
            metavar="<token>",
            help="Token for AMS authentication"
        )
        ...
        parser.add_argument(
            "--ams-key",
            metavar="<keyfile>",
            help="Key file for AMS authentication"
        )
    ...
    def _get_ams_token(self):
        ...
        url = ("https://{0}:8443/v1/service-types/ams/hosts/"
               "{0}:authx509".format(self.opts.ams_host))
    ...
    def publish(self, output):
        ...
        url = ("https://{0}/v1/projects/{2}/topics/{3}"
               ":publish?key={1}".format(self.opts.ams_host,
                                         token,
