...
##
provider = {}
provider['site_name']    = 'SZTAKI'
provider['production_level'] = 'production' # e.g. 'production'
provider['full_bdii_schema'] = 'true' #Produce full BDII schema (to import from scratch). If set to false, the following site information (up to the site_bdii_port) will be ignored
...
def generaldomain (site_name, web_site_name, ngi, country, site_latitude, site_longitude, general_contact, security_contact, user_contact, sysadmin_contact):
...
objectClass: GLUE2Group
GLUE2GroupID: resource
"""%(site_name, site_name, site_name, web_site_name, ngi, site_name,site_name,site_name,country,site_longitude,site_latitude,site_name, site_name,site_name,general_contact,site_name,site_name, site_name,site_name,sysadmin_contact,site_name,site_name, site_name,site_name,security_contact,site_name,site_name, site_name,site_name,user_contact,site_name,site_name,site_name)
    return text

def clouddomain(site_name):
	'''  '''
	text = """dn: GLUE2GroupID=cloud,GLUE2DomainID=%s,o=glue
objectClass: GLUE2Group
GLUE2GroupID: cloud
"""%(site_name)
	return text
...
#### OUTPUT
if provider['full_bdii_schema']!='false':
	print headers()
	print generaldomain(provider['site_name'],provider['www'],provider['affiliated_ngi'],provider['country'],provider['site_latitude'],provider['site_longitude'],provider['general_contact'],provider['security_contact'],provider['user_support_contact'],provider['sysadmin_contact'])
	
	print localbdii(provider['site_name'],provider['production_level'],provider['site_bdii_host'],provider['site_bdii_port'])

print clouddomain(provider['site_name'])
